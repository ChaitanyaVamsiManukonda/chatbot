...
// Determine data directory - handle both local and serverless environments
const getDataPath = () => {
  // In Netlify Functions, use /tmp for temporary file storage
  if (process.env.NETLIFY_FUNCTIONS_ORIGIN) {
    return '/tmp/chatbot-index.json';
  }
  // In local development, use the data directory
  return INDEX_PATH;
};

// Check if we're in a serverless environment (Netlify, Lambda, etc.)
const isServerless = () => {
  return process.env.NETLIFY_FUNCTIONS_ORIGIN || process.env.AWS_LAMBDA_FUNCTION_NAME || process.env.VERCEL;
};
...
async function buildIndex(docs) {
  ...
  const indexedDocs = docs.map(d => {
    ...
    // Preserve optional metadata (source type / url / original payload)
    const meta = d.meta || { type: d.type || 'text', source: d.source || d.source_url || d.url || null };
    return { id: d.id, title: d.title || '', text: d.text, tf, termsCount: terms.length, meta };
  });
  const idf = {};
  ...
  // Write to environment-aware path
  const dataPath = getDataPath();
  try {
    // If using the data directory (local dev), ensure it exists
    if (dataPath === INDEX_PATH) {
      ensureDataDir();
    }
    fs.writeFileSync(dataPath, JSON.stringify(index, null, 2), 'utf8');
    console.log('Index saved to:', dataPath);
  } catch (err) {
    console.error('Failed to save index locally:', err.message, '. Attempting S3 upload only.');
  }
  ...
  if (s3Client && PutObjectCommand) {
    ...
    } else if (isServerless() && !s3Client) {
      console.warn('Running in serverless environment without S3 configured. Index will be stored in /tmp and lost after function execution. Configure S3_BUCKET for persistent storage.');
  }
...
async function loadIndex() {
  ...
  try {
    ...
    console.warn('Failed to load index from local storage:', e.message);
    return null;
  }
}
...
async function search(query, topK = 5) {
  ...
  const scored = index.docs.map(doc => ({ id: doc.id, title: doc.title, text: doc.text, meta: doc.meta || null, score: scoreDoc(tokens, index, doc) }));
